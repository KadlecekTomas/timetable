...
const DEFINITIONS: SheetDefinition[] = [
  {
    ...
    columns: [
      ...
      col("monday_periods", "Pondělí – počet hodin *", true, 22, "Celé číslo 1–16.", whole(1, 16)),
      col("tuesday_periods", "Úterý – počet hodin *", true, 22, "Celé číslo 1–16.", whole(1, 16)),
      col("wednesday_periods", "Středa – počet hodin *", true, 22, "Celé číslo 1–16.", whole(1, 16)),
      col("thursday_periods", "Čtvrtek – počet hodin *", true, 22, "Celé číslo 1–16.", whole(1, 16)),
      col("friday_periods", "Pátek – počet hodin *", true, 22, "Celé číslo 1–16.", whole(1, 16)),
    ],
  },
  {
    ...
    columns: [
      ...
      col("target_weekly_load", "Cílový počet hodin týdně *", true, 25, "Součet výukových vazeb.", whole(0, 60)),
      col("min_weekly_load", "Minimální počet hodin", false, 23, "Volitelné spodní omezení.", whole(0, 60)),
      col("max_weekly_load", "Maximální počet hodin", false, 23, "Volitelné horní omezení.", whole(0, 60)),
  ...
  },
  {
    ...
    tabColor: "FF9B78DB",
    columns: [
      col("subject_code", "Zkratka předmětu *", true, 20, "Například M, CJ nebo AJ."),
      col("subject_name", "Název předmětu *", true, 28, "Celý název předmětu."),
      col("default_room_type", "Výchozí typ učebny", false, 25, "Například TĚLOCVIČNA nebo LABORATOŘ."),
    ],
  },
  {
    ...
    legacyName: "Učebny",
    title: "Učebny",
    instruction: "Zadejte běžné i odborné učebny. Typy používejte konzistentně.",
    tabColor: "FFF2A65A",
    columns: [
      col("room_code", "Zkratka učebny *", true, 18, "Například 101 nebo TV1."),
      col("room_name", "Název učebny *", true, 30, "Zobrazovaný název."),
      col("room_type", "Typ učebny", false, 24, "Například BĚŽNÁ nebo LABORATOŘ."),
      col("capacity", "Kapacita", false, 14, "Volitelný počet míst.", whole(1, 500)),
    ],
  },
  {
    ...
    columns: [
      ...
      col("subject_code", "Předmět *", true, 16, "Zkratka z listu 3. Předměty."),
      col("teacher_code", "Učitel *", true, 16, "Zkratka z listu 1. Učitelé."),
      col("group", "Skupina *", true, 18, "Pro běžnou výuku zvolte Celá třída.", list(Object.keys(MAPS.group)), MAPS.group),
      col("weekly_periods", "Hodin týdně *", true, 17, "Celková týdenní dotace.", whole(1, 30)),
      col("lesson_shape", "Rozložení hodin *", true, 34, "Jednotlivě, ve dvojhodinách nebo kombinovaně.", list(Object.keys(MAPS.shape)), MAPS.shape),
      col("double_periods_count", "Počet dvojhodin", false, 19, "Používá se u kombinovaného rozložení.", whole(0, 15)),
      col("required_room", "Konkrétní učebna", false, 22, "Zkratka z listu 4. Učebny."),
      col("required_room_type", "Požadovaný typ učebny", false, 25, "Pokud není nutná jedna konkrétní učebna."),
      col("max_per_day", "Max. hodin za den", false, 20, "Nejvyšší počet této výuky za den.", whole(1, 8)),
      col("min_day_gap", "Min. rozestup mezi dny", false, 23, "Celé dny mezi opakováními.", whole(0, 4)),
    ],
  },
  {
    ...
    tabColor: "FFE76F51",
    columns: [
      col("entity_type", "Koho / čeho se týká *", true, 23, "Učitel, třída nebo učebna.", list(Object.keys(MAPS.entity)), MAPS.entity),
      col("entity_code", "Zkratka *", true, 18, "Zkratka konkrétní položky."),
      col("day", "Den *", true, 16, "Den v týdnu.", list(Object.keys(MAPS.day)), MAPS.day),
      col("period", "Vyučovací hodina *", true, 21, "Čísluje se od 1.", whole(1, 16)),
      col("kind", "Pravidlo *", true, 18, "Nemůže je tvrdý zákaz; ostatní jsou preference.", list(Object.keys(MAPS.availability)), MAPS.availability),
      col("weight", "Síla preference", false, 18, "Číslo 1–100; u zákazu prázdné.", whole(1, 100)),
  ...
  },
  {
    ...
    tabColor: "FF6C757D",
    columns: [
      col("assignment_code", "Kód výukové vazby *", true, 25, "Kód z listu 5. Kdo co učí."),
      col("block_index", "Pořadí bloku *", true, 18, "První blok je 0, druhý 1.", whole(0, 100)),
      col("day", "Den *", true, 16, "Den v týdnu.", list(Object.keys(MAPS.day)), MAPS.day),
      col("start_period", "Začátek – hodina *", true, 21, "Čísluje se od 1.", whole(1, 16)),
      col("duration", "Délka *", true, 14, "1 = hodina, 2 = dvojhodina.", whole(1, 4)),
      col("room_code", "Učebna", false, 18, "Volitelná zkratka učebny."),
      col("locked", "Zamknout", false, 16, "Ano = hodina se nesmí přesunout.", list(Object.keys(MAPS.boolean)), MAPS.boolean),
    ],
  },
];
...
function styleSheet(worksheet: Worksheet, definition: SheetDefinition) {
  ...
  worksheet.views = [{ state: "frozen", ySplit: CLIENT_TEMPLATE_HEADER_ROW, showGridLines: false }];
  ...
  instruction.fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLORS.paleBlue } };
  ...
  legend.value = "Žluté sloupce jsou povinné. Bílé jsou volitelné. Názvy listů ani záhlaví neměňte.";
  legend.font = { bold: true };
  legend.fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLORS.paleYellow } };
  ...
  definition.columns.forEach((column, index) => {
    ...
    header.fill = { type: "pattern", pattern: "solid", fgColor: { argb: column.required ? COLORS.blue : COLORS.gray } };
    header.alignment = { vertical: "middle", wrapText: true };
    header.note = column.help;

    for (let row = CLIENT_TEMPLATE_FIRST_DATA_ROW; row <= CLIENT_TEMPLATE_LAST_DATA_ROW; row += 1) {
      const cell = worksheet.getCell(row, columnNumber);
      cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: column.required ? COLORS.paleYellow : COLORS.white } };
...
function createGuide(workbook: ExcelJS.Workbook) {
  const sheet = workbook.addWorksheet(CLIENT_TEMPLATE_SHEET_NAMES.guide, { properties: { tabColor: { argb: COLORS.green } } });
  sheet.views = [{ showGridLines: false }];
  sheet.columns = [{ width: 4 }, { width: 20 }, { width: 22 }, { width: 22 }, { width: 22 }, { width: 22 }, { width: 22 }, { width: 4 }];
  ...
  sheet.getCell("B5").value = "Vyplňte listy zleva doprava. Žlutá pole jsou povinná a většina voleb je připravena česky.";
  sheet.getCell("B5").alignment = { horizontal: "center", vertical: "middle", wrapText: true };
  sheet.getRow(5).height = 38;

  const steps = [
    ["1", "Nastavení", "Zkontrolujte školní rok a počet hodin v jednotlivých dnech."],
    ["2", "Seznamy", "Vyplňte učitele, třídy, předměty a učebny."],
    ["3", "Kdo co učí", "Propojte učitele, třídy a předměty a určete týdenní dotaci."],
  ...
  for (const [number, heading, text] of steps) {
    ...
    numberCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLORS.blue } };
    ...
    sheet.getCell(row, 3).fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLORS.paleBlue } };
  ...
  sheet.getCell("B24").fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLORS.paleYellow } };
  [
...
function createExamples(workbook: ExcelJS.Workbook) {
  const sheet = workbook.addWorksheet(CLIENT_TEMPLATE_SHEET_NAMES.examples, { properties: { tabColor: { argb: COLORS.yellow } } });
  sheet.views = [{ showGridLines: false }];
  ...
  title.alignment = { horizontal: "center", vertical: "middle" };
  const sections = [
    [5, "Učitel", ["Zkratka", "Jméno", "Příjmení", "Hodin týdně", "Předměty", "Třídy"], ["NOV", "Jan", "Novák", 18, "M,F", "6A,7A"]],
    [10, "Kdo co učí", ["Kód vazby", "Třída", "Předmět", "Učitel", "Skupina", "Hodin týdně", "Rozložení", "Učebna"], ["6A-M-NOV", "6A", "M", "NOV", "Celá třída", 4, "Jednotlivé hodiny", "101"]],
    [15, "Nedostupnost", ["Koho se týká", "Zkratka", "Den", "Hodina", "Pravidlo", "Síla", "Poznámka"], ["Učitel", "NOV", "Pátek", 7, "Nemůže", "", "Porada"]],
  ] as const;
  for (const [startRow, heading, headers, values] of sections) {
    ...
    sheet.getCell(startRow, 1).font = { bold: true, color: { argb: COLORS.white } };
    sheet.getCell(startRow, 1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLORS.blue } };
    headers.forEach((header, index) => {
      const cell = sheet.getCell(startRow + 1, index + 1);
      cell.value = header;
      cell.font = { bold: true };
      cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLORS.paleYellow } };
      cell.alignment = { wrapText: true };
    });
    values.forEach((value, index) => {
      const cell = sheet.getCell(startRow + 2, index + 1);
      cell.value = value;
      cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLORS.paleGreen } };
      cell.alignment = { wrapText: true };
    });
  }
}
...
export async function createClientImportTemplate(): Promise<Uint8Array> {
  ...
  workbook.getWorksheet(CLIENT_TEMPLATE_SHEET_NAMES.settings)!.getRow(CLIENT_TEMPLATE_FIRST_DATA_ROW).values = [undefined, "2026/2027", 8, 8, 8, 8, 7];
  workbook.views = [{ activeTab: 0 }];
  return new Uint8Array(await workbook.xlsx.writeBuffer());
}

function normalizeLabel(value: string): string {
  return value.replace(/\s*\*\s*$/, "").replace(/\s+/g, " ").trim().toLocaleLowerCase("cs-CZ");
}

function findHeader(worksheet: Worksheet, definition: SheetDefinition) {
  const labels = new Map(definition.columns.map((column) => [normalizeLabel(column.label), column.key]));
  for (let row = 1; row <= Math.min(12, worksheet.actualRowCount); row += 1) {
    const found = new Map<string, number>();
    for (let column = 1; column <= worksheet.columnCount; column += 1) {
      const key = labels.get(normalizeLabel(worksheet.getCell(row, column).text));
      if (key) found.set(key, column);
    }
    if (definition.columns.every((column) => found.has(column.key))) return { row, columns: found };
  }
  return null;
}

function mapCell(value: CellValue, valueMap?: Record<string, string>): CellValue {
...
function friendlyText(value: string | null): string | null {
  ...
  for (const mapping of Object.values(MAPS)) {
    for (const [friendly, technical] of Object.entries(mapping)) replacements.push([technical, friendly]);
  }
  return replacements.sort(([a], [b]) => b.length - a.length).reduce((text, [technical, friendly]) => text.replaceAll(technical, friendly), value);
}

function remapIssues(issues: ImportIssueDraft[]): ImportIssueDraft[] {
  const definitions = new Map(DEFINITIONS.map((definition) => [definition.legacyName, definition]));
  return issues.map((issue) => {
    ...
    return {
      ...
      row: issue.row == null || issue.row === 1 ? issue.row : issue.row + CLIENT_TEMPLATE_FIRST_DATA_ROW - 2,
...
async function normalizeClientWorkbook(workbook: ExcelJS.Workbook): Promise<Uint8Array | null> {
  const version = workbook.getWorksheet("Metadata")?.getCell("B1").text.trim() ?? "";
  ...
  for (const definition of DEFINITIONS) {
    ...
    if (!header) {
      target.getRow(1).values = [undefined, ...definition.columns.map((column) => `missing_${column.key}`)];
      continue;
    }
    for (let row = header.row + 1; row <= source.actualRowCount; row += 1) {
      const values = definition.columns.map((column) => mapCell(source.getCell(row, header.columns.get(column.key)!).value, column.valueMap));
      if (values.some((value) => value != null && String(value).trim() !== "")) target.addRow(values);
    }
  }
  return new Uint8Array(await legacy.xlsx.writeBuffer());
}

export async function analyzeClientImportWorkbook(buffer: ArrayBuffer | Uint8Array): Promise<ImportAnalysis> {
  const workbook = new ExcelJS.Workbook();
  try {
  ...
  return { ...analysis, templateVersion: CLIENT_IMPORT_TEMPLATE_VERSION, issues: remapIssues(analysis.issues) };
}
