...
export async function GET(request: Request) {
  ...
  try {
    ...
    const leftByBlock = new Map(left.lessons.map((lesson) => [lesson.block_id, lesson]));
    const rightByBlock = new Map(right.lessons.map((lesson) => [lesson.block_id, lesson]));
    const blockIds = [...new Set([...leftByBlock.keys(), ...rightByBlock.keys()])].sort();
    const changes = blockIds.map((blockId) => {
      const before = leftByBlock.get(blockId);
      const after = rightByBlock.get(blockId);
      if (!before) return { blockId, change: "ADDED" as const, before: null, after };
      if (!after) return { blockId, change: "REMOVED" as const, before, after: null };
      const moved =
        before.day !== after.day ||
        before.period !== after.period ||
        before.room_id !== after.room_id ||
        before.duration !== after.duration;
      return { blockId, change: moved ? ("MOVED" as const) : ("UNCHANGED" as const), before, after };
    });

    return NextResponse.json({
      left: { id: left.version.id, name: left.version.name, revision: left.version.revision },
      right: { id: right.version.id, name: right.version.name, revision: right.version.revision },
      summary: {
