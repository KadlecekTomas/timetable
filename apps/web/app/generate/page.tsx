...
import { AlertTriangle, CheckCircle2, Clock3, Play, RefreshCw, Square } from "lucide-react";
...
interface RunView {
  ...
  candidateVersion: { id: string; name: string; qualityScore: number | null } | null;
  explanation?: unknown;
}
...
export default function GeneratePage() {
  ...
  const load = useCallback(async () => {
    ...
    const [readinessResponse, runsResponse] = await Promise.all([
      fetch(`/api/school-years/${schoolYearId}/readiness`, { cache: "no-store" }),
      fetch(`/api/school-years/${schoolYearId}/generation-runs`, { cache: "no-store" }),
    ]);
    if (readinessResponse.ok) setReadiness((await readinessResponse.json()) as ReadinessReport);
    if (runsResponse.ok) {
  ...
  async function start() {
    ...
    try {
      ...
      if (!response.ok) {
        if (payload.error?.details?.readiness) setReadiness(payload.error.details.readiness);
        throw new Error(payload.error?.message ?? "Generování se nepodařilo zařadit.");
      }
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Generování se nepodařilo zařadit.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function cancel(runId: string) {
    const response = await fetch(`/api/generation-runs/${runId}`, { method: "DELETE" });
    if (!response.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-semibold text-text-primary">
                {readiness?.ready ? "Předletová kontrola prošla" : "Předletová kontrola blokuje běh"}
              </h2>
        ...
        <article className="rounded-xl border border-border bg-surface p-5">
          <label htmlFor="time-limit" className="text-sm font-medium text-text-primary">
          ...
          <Button className="mt-4 w-full" onClick={() => void start()} disabled={busy || !readiness?.ready}>
      ...
      <section className="overflow-hidden rounded-xl border border-border bg-surface">
        <div className="border-b border-border px-5 py-4">
          ...
          <p className="mt-1 text-xs text-text-muted">Immutable vstupy, lifecycle a kandidátní verze.</p>
        </div>
        {runs.length ? (
          <div className="divide-y divide-border">
            {runs.map((run) => (
              <div key={run.id} className="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="truncate font-mono text-xs text-text-primary">{run.id}</p>
                      <StatusBadge tone={runTone(run.status)}>{run.status}</StatusBadge>
                      {run.qualityScore != null ? (
                        <StatusBadge tone="success">Skóre {run.qualityScore}/100</StatusBadge>
                      ) : null}
                    </div>
                    <p className="mt-1 text-xs text-text-muted">
                      Vytvořeno {new Date(run.createdAt).toLocaleString("cs-CZ")}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {["QUEUED", "RUNNING"].includes(run.status) ? (
                    <Button variant="outline" size="sm" onClick={() => void cancel(run.id)}>
          ...
          <div className="p-8 text-center text-sm text-text-muted">Zatím nebyl spuštěn žádný běh.</div>
        )}
      </section>
    </div>
  );
