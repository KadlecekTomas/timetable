...
function targetClassCodes(row: TeachingPlanRow): string[] {
  return [row.classCode, ...(row.additionalClassCodes ?? [])].sort(
    (left, right) => left.localeCompare(right, "cs-CZ", { numeric: true }),
  );
}

test(
  "JAZ2 merges every class in grades 8 and 9 even when Špánková has a 12-hour staffing load",
  () => {
    ...
    assert.deepEqual(targetClassCodes(languageRows[0]!), [
      "8.A",
      "8.B",
      "8.C",
    ]);
    assert.deepEqual(targetClassCodes(languageRows[1]!), [
      "9.A",
      "9.B",
      "9.C",
    ]);
