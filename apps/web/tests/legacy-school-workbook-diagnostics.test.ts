...
test("diagnostic: serialized legacy workbook keeps its signature and class block", async () => {
  ...
  assert.equal(reopened.getWorksheet("Úvazky 20252026")?.actualRowCount, 44);
  assert.equal(reopened.getWorksheet("Úvazky 20252026")?.getCell("C41").text, "8.A");
